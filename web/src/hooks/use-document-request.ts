...
import { shouldInvalidateChunkListForDocumentTransition } from './use-document-request.utils';
...
export const useFetchDocumentList = (loop = true) => {
  ...
  const previousDocsRef = useRef<IDocumentInfo[]>([]);
  ...
  useEffect(() => {
    ...
    previousDocsRef.current = data.docs;
    setDocs(data.docs);
