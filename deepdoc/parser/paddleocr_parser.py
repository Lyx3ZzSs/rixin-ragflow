...
AlgorithmType = Literal["PaddleOCR-VL", "PP-OCRv5", "PP-StructureV3", "PaddleOCR-VL-1.5", "PaddleOCR-VL-1.6"]
...
class PaddleOCRParser(RAGFlowPdfParser):
    ...
    def parse_pdf(
        ...
        **kwargs: Any,
        ...
        if self._is_jobs_api(cfg.api_url):
            result = self._send_jobs_request(filepath, data_bytes, cfg, callback)
        else:
    ...
    @staticmethod
    def _is_jobs_api(api_url: str) -> bool:
        return "/jobs" in api_url.rstrip("/")

    def _build_optional_payload(self, config: PaddleOCRConfig) -> dict[str, Any]:
        optional_payload: dict[str, Any] = {}
        algorithm_mapping = self._ALGORITHM_FIELD_MAPPINGS.get(config.algorithm, {})
        for param_key, param_value in config.algorithm_config.items():
            if param_value is not None and param_key in algorithm_mapping:
                optional_payload[algorithm_mapping[param_key]] = param_value

        explicit_optional_payload = config.additional_params.get("optionalPayload") or config.additional_params.get("optional_payload")
        if isinstance(explicit_optional_payload, dict):
            optional_payload.update(explicit_optional_payload)

        return optional_payload

    def _jobs_headers(self, config: PaddleOCRConfig) -> dict[str, str]:
        headers = {"Client-Platform": "ragflow"}
        if config.access_token:
            headers["Authorization"] = f"bearer {config.access_token}"
        return headers

    def _send_jobs_request(
        self,
        filepath: str | PathLike[str],
        data: bytes,
        ...
        """Submit to PaddleOCR v2 jobs API, poll completion, and normalize jsonl result."""

        headers = self._jobs_headers(config)
        optional_payload = self._build_optional_payload(config)
        form_data = {
        ...
        self.logger.info("[PaddleOCR] submitting v2 jobs request")
        if callback:
        ...
        try:
            with BytesIO(data) as file_obj:
                ...
                resp = requests.post(config.api_url, headers=headers, data=form_data, files=files, timeout=config.request_timeout)
            resp.raise_for_status()
            job_id = resp.json()["data"]["jobId"]
        except Exception as exc:
            if callback:
                callback(-1, f"[PaddleOCR] job submit failed: {exc}")
            raise RuntimeError(f"[PaddleOCR] job submit failed: {exc}") from exc

        if callback:
            callback(0.2, f"[PaddleOCR] job submitted: {job_id}")

        result_url = self._poll_jobs_result(config.api_url, job_id, headers, config, callback)
        return self._fetch_jobs_jsonl(result_url, config)

    def _poll_jobs_result(
        self,
        api_url: str,
        job_id: str,
        ...
        while True:
            if time.monotonic() - start_time > config.request_timeout:
                raise RuntimeError(f"[PaddleOCR] job polling timeout after {config.request_timeout}s")

            try:
                resp = requests.get(job_url, headers=headers, timeout=config.request_timeout)
                resp.raise_for_status()
                data = resp.json().get("data", {})
            except Exception as exc:
                if callback:
                    callback(-1, f"[PaddleOCR] job polling failed: {exc}")
                raise RuntimeError(f"[PaddleOCR] job polling failed: {exc}") from exc

            state = data.get("state")
            if state == "done":
                result_url = data.get("resultUrl", {}).get("jsonUrl")
                if not result_url:
                    raise RuntimeError("[PaddleOCR] job done but jsonUrl missing")
                if callback:
                    callback(0.75, "[PaddleOCR] job done")
                return result_url
            ...
            if callback:
                ...
                if total_pages:
                    callback(0.5, f"[PaddleOCR] job {state}, pages: {extracted_pages}/{total_pages}")
                else:
                    callback(0.4, f"[PaddleOCR] job {state}")

            time.sleep(max(config.poll_interval, 1))

    def _fetch_jobs_jsonl(self, jsonl_url: str, config: PaddleOCRConfig) -> dict[str, Any]:
        try:
            resp = requests.get(jsonl_url, timeout=config.request_timeout)
            resp.raise_for_status()
        except Exception as exc:
            raise RuntimeError(f"[PaddleOCR] fetch jsonl failed: {exc}") from exc
        ...
        for line in resp.text.strip().splitlines():
            ...
            try:
                ...
                raise RuntimeError(f"[PaddleOCR] invalid jsonl line: {exc}") from exc

            result = item.get("result", {})
            page_results = result.get("layoutParsingResults", [])
            if isinstance(page_results, list):
                layout_parsing_results.extend(page_results)
        ...
        if algorithm in SUPPORTED_PADDLEOCR_ALGORITHMS:
            ...
            for page_idx, layout_result in enumerate(layout_parsing_results):
                ...
                if not parsing_res_list:
                    ...
                    markdown_text = _remove_images_from_markdown(markdown_text.strip())
                    if markdown_text:
                        tag = f"@@{page_idx + 1}\t0\t0\t0\t0##"
                        if parse_method in {"manual", "pipeline"}:
                            sections.append((markdown_text, "markdown", tag))
                        elif parse_method == "paper":
                            sections.append((markdown_text + tag, "markdown"))
                        else:
                            sections.append((markdown_text, tag))
                    continue
