...
1. 第二阶段采用“产品闭环优先”路线，优先交付历史、条件编辑、导出和反馈。
2. Redis 继续用于运行中任务的短期状态更新，数据库作为历史任务、结果、证据、导出和反馈的事实来源。
3. 条件编辑发生在任务创建前：先解析 Prompt，再由用户确认或修改结构化条件，最后提交筛选任务。
...
- `edited_conditions`: 用户确认后的结构化条件，JSON。
...
第一阶段请求格式继续兼容。未提交 `conditions` 时，后端使用 Prompt 自动解析结果。
...
用户输入 Prompt 后，前端先调用解析接口并展示条件确认区。
...
用户点击“开始筛选”后，前端把编辑后的条件提交到创建任务接口。
...
- 风险：条件编辑 UI 过重，影响第一阶段简洁体验。
  - 缓解：默认折叠高级字段，只展示条件启用、名称和值；关键词和证据策略放入高级编辑区。

- 风险：Word 导出版式投入过大。
  - 缓解：第二阶段只做朴素、可读、可打印版本，正式报告留到第三阶段。

- 风险：反馈采集后短期看不到收益。
  - 缓解：第二阶段只承诺数据沉淀和统计，调优策略进入后续阶段。
