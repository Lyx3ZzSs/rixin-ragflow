...
from common.parser_config_utils import DEFAULT_LAYOUT_RECOGNIZER, normalize_layout_recognizer
from common.paddleocr_config import collect_paddleocr_config
    ...
    **kwargs,
    ...
    if tenant_id:
        ...
        if paddleocr_llm_name:
            ocr_model_config = None
            try:
                ocr_model_config = get_model_config_from_provider_instance(tenant_id, LLMType.OCR, paddleocr_llm_name)
            except LookupError as e:
                logging.error(f"Failed to resolve LLMBundle PaddleOCR ({paddleocr_llm_name}): {e}")

                ocr_model_config = _paddleocr_model_config_from_service_config(paddleocr_llm_name)
                if ocr_model_config:
                    logging.info("[PaddleOCR] fallback to service_conf.yaml config")
            except Exception as e:
                logging.error(f"Failed to resolve LLMBundle PaddleOCR ({paddleocr_llm_name}): {e}")
            ...
            if ocr_model_config:
                try:
                    ...
                    logging.error(f"Failed to parse pdf via PaddleOCR ({paddleocr_llm_name}): {e}")
                    if callback:
...
def _build_paddleocr_model_config(config: dict | None, paddleocr_llm_name: str | None = None) -> dict | None:
    if not config:
        return None

    model_name = config.get("PADDLEOCR_ALGORITHM") or "PaddleOCR-VL"
    if paddleocr_llm_name:
        model_name = paddleocr_llm_name.split("@", 1)[0] or model_name
...
def _paddleocr_model_config_from_service_config(paddleocr_llm_name: str | None = None) -> dict | None:
    return _build_paddleocr_model_config(collect_paddleocr_config(), paddleocr_llm_name)
...
def chunk(filename, binary=None, from_page=0, to_page=MAXIMUM_PAGE_NUMBER, lang="Chinese", callback=None, **kwargs):
    ...
    parser_config = kwargs.get("parser_config", {"chunk_token_num": 512, "delimiter": "\n!?。；！？", "layout_recognize": DEFAULT_LAYOUT_RECOGNIZER, "analyze_hyperlink": True})
    ...
    if re.search(r"\.docx$", filename, re.IGNORECASE):
        ...
        layout_recognizer, parser_model_name = normalize_layout_recognizer(parser_config.get("layout_recognize", DEFAULT_LAYOUT_RECOGNIZER))
        ...
        if isinstance(layout_recognizer, bool):
            layout_recognizer = DEFAULT_LAYOUT_RECOGNIZER if layout_recognizer else "PlainText"
        ...
        layout_recognizer = parser_config.get("layout_recognize", DEFAULT_LAYOUT_RECOGNIZER)
